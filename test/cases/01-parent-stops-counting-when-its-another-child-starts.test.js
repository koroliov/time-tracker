...
tp('', async t => {
  ...
  t.equal(ttBillableCurrent, '0%');
  ttBillableToTargetPercent =
    await u.timeTrackerBillableCurrentToTargetPercentGet();
  t.equal(ttBillableToTargetPercent, '0%');

  await u.commonTimeEntryAddNew([1,]);
  await u.timeEntrySetBillable([1, 2,]);
  await u.timeEntryStart([1, 2,]);

  date += 60000;
  await u.injectDate(date);
  await u.pause(1500);

  totalTime = await u.timeEntryTotalTimeGet([1, 2,]);
  t.equal(totalTime, '0d 0h 1m 0s');
  ownTime = await u.timeEntryOwnTimeGet([1, 2,]);
  t.equal(ownTime, '0d 0h 1m 0s');
  billableTime = await u.timeEntryBillableTimeGet([1, 2,]);
  t.equal(billableTime, '0d 0h 1m 0s');
  billablePercent = await u.timeEntryBillableTimePercentGet([1, 2,]);
  t.equal(billablePercent, '100%');
